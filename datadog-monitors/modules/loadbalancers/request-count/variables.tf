...
variable "name" {
  ...
  type        = string
  default     = "AWS ApplicationLB: Number of requests handled by the load balancer"
}
variable "message" {
  description = "(required) - A message to include with notifications for this monitor.\n\nEmail notifications can be sent to specific users by using the same `@username` notation as events."
  type        = string
  default     = <<MESSAGE
  The number of requests made to the ELB.

  {{#is_warning}} Warning Triggered for {{loadbalancer.name}} in {{region.name}} is over {{warn_threshold}}.  {{/is_warning}}

  {{#is_warning_recovery}} {{loadbalancer.name}} in {{region.name}} Has Recovered and is below {{warn_threshold}}.  {{/is_warning_recovery}}

  {{#is_alert}} Alert Triggered for {{loadbalancer.name}} in {{region.name}} is over {{threshold}}.  {{/is_alert}}

  {{#is_alert_recovery}} {{loadbalancer.name}} in {{region.name}} Has Recovered and is below {{threshold}}.  {{/is_alert_recovery}}
