...
variable "name" {
  ...
  type        = string
  default     = "AWS NAT: Monitor outgoing traffic from NAT gateway"
}
variable "message" {
  description = "(required) - A message to include with notifications for this monitor.\n\nEmail notifications can be sent to specific users by using the same `@username` notation as events."
  type        = string
  default     = <<MESSAGE
  AWS NAT: Monitor outgoing traffic from NAT gateway
  Experiencing more than 80% of data loss during NAT gateway processing.
