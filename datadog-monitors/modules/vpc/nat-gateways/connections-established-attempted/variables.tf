...
variable "name" {
  ...
  type        = string
  default     = "AWS NAT: ConnectionEstablishedCount vs. ConnectionAttemptCount"
}
variable "message" {
  description = "(required) - A message to include with notifications for this monitor.\n\nEmail notifications can be sent to specific users by using the same `@username` notation as events."
  type        = string
  default     = <<MESSAGE
  AWS NAT: ConnectionEstablishedCount vs. ConnectionAttemptCount
  More than 80% of connections to NAT gateway attempted to establish new connections for which there was no response.
