...
variable "name" {
  ...
  type        = string
  default     = "AWS NAT:  Monitor incoming traffic into NAT gateway"
}
variable "message" {
  description = "(required) - A message to include with notifications for this monitor.\n\nEmail notifications can be sent to specific users by using the same `@username` notation as events."
  type        = string
  default     = <<MESSAGE
  AWS NAT:  Monitor incoming traffic into NAT gateway
  Experiencing more than 80% of data loss during NAT gateway processing or or traffic being actively blocked by the NAT gateway
