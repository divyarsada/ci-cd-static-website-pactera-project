...
variable "name" {
  ...
  type        = string
  default     = "AWS NAT: number of packets dropped by the NAT gateway"
}
variable "message" {
  description = "(required) - A message to include with notifications for this monitor.\n\nEmail notifications can be sent to specific users by using the same `@username` notation as events."
  type        = string
  default     = <<MESSAGE
  AWS NAT: number of packets dropped by the NAT gateway
  A value greater than zero may indicate an ongoing transient issue with the NAT gateway. If this value is high, see the AWS service health dashboard https://status.aws.amazon.com/

  {{#is_warning}} Warning Triggered for {{natgatewayid.name}}  in {{region.name}} is over {{warn_threshold}}.  {{/is_warning}}

  {{#is_warning_recovery}} {{natgatewayid.name}}  in {{region.name}} Has Recovered and is below {{warn_threshold}}.  {{/is_warning_recovery}}

  {{#is_alert}} Alert Triggered for {{natgatewayid.name}}  in {{region.name}} is over {{threshold}}.  {{/is_alert}}

  {{#is_alert_recovery}} {{natgatewayid.name}}  in {{region.name}} Has Recovered and is below {{threshold}}.  {{/is_alert_recovery}}
